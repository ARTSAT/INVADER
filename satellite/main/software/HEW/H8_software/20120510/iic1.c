...
* P5B4-SCL,P5B5-SDAとして使用
*
******************************************************************************/
#include "iodefine.h"
#include "iic.h"

#define eepstop		IIC1_EEPSTOP
#define	eepread		IIC1_EEPREAD
#define eepwrite	IIC1_EEPWRITE
#define	eepreset	IIC1_EEPRESET
#define eepinit		IIC1_EEPINIT
#define	eepwait		IIC1_EEPWAIT
#define	writeb		IIC1_WRITEB
#define readb		IIC1_READB
#define eepstart	IIC1_EEPSTART
#define eepchange   IIC1_EEPCHANGE

#define		SCL_H	(P2DDR = P2_DDR = P2_DDR &= ~16)	// P2DDR.BIT.B4のみを0にする(入力設定) ~16=1110 1111
#define		SCL_L	(P2DDR = P2_DDR |=  16)				// P2DDR.BIT.B4のみを1にする(出力設定)  16=0001 0000
#define		SDA_H	(P2DDR = P2_DDR &= ~32)				// P2DDR.BIT.B5のみを0にする(入力設定) ~32=1101 1111
#define		SDA_L	(P2DDR = P2_DDR |=  32)				// P2DDR.BIT.B5のみを1にする(出力設定)  32=0010 0000
...
static void writeb(unsigned char dat)
{
	...
	for (i=0; i<=7; i++){
		...
		SCL_H;								// SCLがHigh
		eepwait(CYCLE);
		while ((P2DR.BYTE & 16) == 0);		// SCLがHighで無くなるまでループ
		SCL_L;								// SCL_Low
	}
...
static void readb(unsigned char *dat, unsigned char f)
{
	...
	for (i=0; i<=7; i++){
		SCL_H;
		*dat <<= 1;
		if (P2DR.BYTE & 32) *dat |= 0x01;	// SDAにHighが入力されているならdatに1を返す
		eepwait(CYCLE);
		while ((P2DR.BYTE & 16) == 0);		// SCLがLowになるまで待つ
		SCL_L;
	}
...
void eepread(unsigned long int page, unsigned char *buf, unsigned char s_ad, unsigned char byte)
{
	...
	P2DR.BYTE &= ~0x30;                  // ~0x30 = 11001111 : データレジスタは出力時に絶対条件として"L"
